-- Crear tabla de vehículos España con datos IDAE en db_movilidad
-- Fuente: https://coches.idae.es/storage/csv/idae-historico-202413.csv
-- Ejecutar en: db_movilidad
-- Creado: 2025-09-09
...
CREATE TABLE IF NOT EXISTS core_vehiculos_espana (
    ...
    consumo_nedc DECIMAL(8,2), -- L/100km
    emisiones_nedc DECIMAL(8,2), -- g CO2/km
    
    -- Consumos WLTP (ciclo actual)
    consumo_mixto_wltp_min DECIMAL(8,2), -- L/100km
    consumo_mixto_wltp_max DECIMAL(8,2), -- L/100km
    consumo_mixto_wltp DECIMAL(8,2), -- L/100km (valor medio)
    
    -- Emisiones CO2 WLTP
    emisiones_co2_mixto_wltp_min DECIMAL(8,2), -- g CO2/km
    emisiones_co2_mixto_wltp_max DECIMAL(8,2), -- g CO2/km
    
    -- Datos eléctricos
    autonomia_electrica INTEGER, -- km (0 si no es eléctrico)
    consumo_electrico DECIMAL(8,2), -- kWh/100km
...
COMMENT ON COLUMN core_vehiculos_espana.emisiones_por_litro IS 'g CO2 por litro de combustible - métrica de eficiencia de combustión';
COMMENT ON COLUMN core_vehiculos_espana.coste_100km_estimado IS 'Coste operativo estimado por 100km según precios energía actuales';

-- Verificar creación
SELECT 'Tabla core_vehiculos_espana creada exitosamente en db_movilidad' as resultado;
